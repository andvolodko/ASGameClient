...
public class Engine extends Sprite implements IUpdatable {
    ...
    public function removeEntityById(entID:int):void {
        for (var i:int = 0; i < entities.length; ++i)
        {
            if (entities[i].getUID() == entID) {
                entities[i].remove();
                return;
            }
        }
    ...
    public function haveEntityWithUID(uid:int):Boolean {
        for (var i:int = 0; i < entities.length; ++i)
        {
            if (entities[i].getUID() == uid) return true;
        }
        return false;
    }
